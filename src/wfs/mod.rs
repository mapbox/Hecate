...
pub fn get_feature(conn: &r2d2::PooledConnection<r2d2_postgres::PostgresConnectionManager>, query: &Query) -> Result<String, HecateError> {
    ...
    let select_type = match query.resulttype {
        ResultType::Results => String::from(r#"
            ST_AsGML(3, ST_Extent(d.geom), 5, 32) AS extent,
            Array_Agg(d.gml) as geoms
        "#),
        ResultType::Hits => String::from("count(*)"),
        ResultType::Invalid => {
        ...
        FROM (
            SELECT
                ...
                Array [
                    ...
                    geo.version::TEXT,
                    geo.props::JSON::TEXT,
                    ST_AsGML(geo.geom)::TEXT,
                    ST_AsGML(3, geo.geom, 5, 32)::TEXT
                ] as gml
            FROM geo
            WHERE
                {geom_filter}
        ...
        Ok(res) => {
            match query.resulttype {
                ...
                _ => {
                    ...
                    let geoms: Vec<Vec<String>> = res.get(0).get(1);
